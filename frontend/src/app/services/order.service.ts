...
export class OrderService {
  ...
  assignToSeller(uuid: string, sellerUsername: string): Observable<HttpResponse<any>> {
    ...
    return this.http
      .put<any>(ApiConfig.apiUrl + '/order/' + uuid + '/assign', null, {params: params, observe: 'response'})
  }

  cancelOrder(uuid: string): Observable<HttpResponse<any>> {
    return this.http
      .put<any>(ApiConfig.apiUrl + '/order/' + uuid + '/cancel', null, {observe: 'response'})
  }

  completeOrder(uuid: string): Observable<HttpResponse<any>> {
    return this.http
      .put<any>(ApiConfig.apiUrl + '/order/' + uuid + '/complete', null, {observe: 'response'})
  }
}
